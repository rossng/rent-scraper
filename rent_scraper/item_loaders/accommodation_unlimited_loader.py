...
def format_price(price_text):
    """Take text representing a price on the Accommodation Unlimited website and return an integer."""
    return int(float(re.sub(r"[^\d\.]", "", price_text.split(" ")[1])))
...
class AccommodationUnlimitedPropertyLoader(ItemLoader):
    ...
    number_bedrooms_in = Compose(MapCompose(strip), TakeFirst(), Split(' '), Get(0), MapCompose(int))
